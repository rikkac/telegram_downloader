...
async def _get_media_meta(
    ...
    if _type in ["voice", "video_note"]:
        ...
        # if len(file_name) >= 10:
        #     file_name = f'{file_name[0:10]}-{str(file_num)}-{file_name[10:]}'
        # else:
        #     file_name += f'-{str(file_num)}-'
        #
        # file_num += f'.{file_format}'
        ...
        gen_file_name = (
            app.get_file_name(message.id, file_name, caption, file_num) + file_name_suffix
        )
    ...
    try:
        for _type in media_types:
            ...
            file_name, temp_file_name, file_format = await _get_media_meta(
                node.chat_id, message, _media, _type, app.file_num
            )
