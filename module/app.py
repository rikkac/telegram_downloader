...
class Application:
    ...
    def assign_config(self, _config: dict) -> bool:
        ...
        if _config.get("file_num") is not None:
            self.file_num = _config["file_num"]
    ...
    def get_file_name(
            self, message_id: int, file_name: Optional[str], caption: Optional[str], file_num: int
    ) -> str:
        """Get file save path prefix.
        ...
        for prefix in self.file_name_prefix:
            if prefix == "message_id":
                ...
                if res != "":
                    res += self.file_name_prefix_split
                # if len(caption) >= 10:
                #     caption = f'{caption[0:10]}-{str(file_num)}-{caption[10:]}'
                # else:
                #     caption += f'-{str(file_num)}-'
                res += f"{caption}"
        if res == "":
            res = f"{message_id}"
